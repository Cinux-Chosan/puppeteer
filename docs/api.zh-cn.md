...
#### elementHandle.hover()
- returns: <[Promise]> Promise which resolves when the element is successfully hovered. （返回 Promise，当元素成功被 hover 时 Promise 返回）

This method scrolls element into view if needed, and then uses [page.mouse](#pagemouse) to hover over the center of the element.
If the element is detached from DOM, the method throws an error.

在必要的情况下该方法会先将元素移入视口，然后使用 [page.mouse](#pagemouse) 方法 hover 到元素中心。如果元素从 DOM 中移除则该方法会抛出错误。

#### elementHandle.isIntersectingViewport()
- returns: <[Promise]<[boolean]>> Resolves to true if the element is visible in the current viewport. （如果元素在当前视口中可见则 Promsie 返回 true）
...
返回对象的 JSON 表示形式。 通过在页面中对对象执行 [`JSON.stringify`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) 然后传递给 Puppeteer，Puppeteer再通过  [`JSON.parse`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse) 解析来获取对象的 JSON。

> **NOTE** The method will throw if the referenced object is not stringifiable.
>
> **提示：** 如果对象不可序列化则该方法抛出异常。

#### elementHandle.press(key[, options])
- `key` <[string]> Name of key to press, such as `ArrowLeft`. See [USKeyboardLayout] for a list of all key names. （需要按下的键名，如 `ArrowLeft`。在 [USKeyboardLayout] 可以查看所有键名）
- `options` <[Object]>
  - `text` <[string]> If specified, generates an input event with this text. （如果指定了该参数，则会使用它生成 input 事件）
  - `delay` <[number]> Time to wait between `keydown` and `keyup` in milliseconds. Defaults to 0. （`keydown` 和 `keyup` 之间的间隔，即按键时长）
- returns: <[Promise]>

Focuses the element, and then uses [`keyboard.down`](#keyboarddownkey-options) and [`keyboard.up`](#keyboardupkey).

将焦点移到该元素， 然后使用 [`keyboard.down`](#keyboarddownkey-options) 和 [`keyboard.up`](#keyboardupkey) 方法产生操作。

If `key` is a single character and no modifier keys besides `Shift` are being held down, a `keypress`/`input` event will also be generated. The `text` option can be specified to force an input event to be generated.

如果 `key` 是单个字符并且没有除了 `Shift` 的其它修饰键被按下，则同时还会创建 `keypress`/`input` 事件。可以通过指定 `text` 选项来强制生成 input 事件。

> **NOTE** Modifier keys DO effect `elementHandle.press`. Holding down `Shift` will type the text in upper case.
>
> **提示：** 修饰键会影响 `elementHandle.press`。按住 `Shift` 会以大写形式输入文本。

#### elementHandle.screenshot([options])
- `options` <[Object]> Same options as in [page.screenshot](#pagescreenshotoptions). （同 [page.screenshot](#pagescreenshotoptions) 的参数）
- returns: <[Promise]<[Buffer]>> Promise which resolves to buffer with captured screenshot. （返回的 Promise 能够返回所捕获屏幕截图的 buffer）
...
在需要的情况下，该方法会将元素移入视口中，然后使用 [page.screenshot](#pagescreenshotoptions) 对元素进行截图。如果元素此时已经从 DOM 中被移除则会抛出错误。

#### elementHandle.tap()
- returns: <[Promise]> Promise which resolves when the element is successfully tapped. Promise gets rejected if the element is detached from DOM. （当元素成功被 tap 时 Promise 返回。如果元素已经从 DOM 中被移除则 Promise 返回拒绝态）
...
该方法在需要的情况下会将元素滚入视图中，然后使用 [touchscreen.tap](#touchscreentapx-y) 轻点元素中心。如果此时元素已经从 DOM 中移除则抛出错误。

#### elementHandle.toString()
- returns: <[string]>

#### elementHandle.type(text[, options])
- `text` <[string]> A text to type into a focused element. （需要输入到焦点元素中的文本）
- `options` <[Object]>
  - `delay` <[number]> Time to wait between key presses in milliseconds. Defaults to 0. （每次敲键的间隔时长，以毫秒为单位。默认为 0）
- returns: <[Promise]>

Focuses the element, and then sends a `keydown`, `keypress`/`input`, and `keyup` event for each character in the text.

将焦点移到元素上，然后会为每个输入的字符发送 `keydown`, `keypress`/`input` 和 `keyup` 事件。

To press a special key, like `Control` or `ArrowDown`, use [`elementHandle.press`](#elementhandlepresskey-options).

如果需要按特殊键，如 `Control` 或 `ArrowDown`，则使用 [`elementHandle.press`](#elementhandlepresskey-options)。
